...
function ResultDisplay({ analysisData, isLoading, shopName, analysisTime, onBackToMain, progressLogs }) {
  ...
  useEffect(() => {
    ...
    const filterCustomerOrders = () => {
      return safeData.customer_based_orders.filter(order => 
        order.customer?.toLowerCase().includes(term) ||
        order.orders?.toLowerCase().includes(term) ||
        order.total_items?.toString().toLowerCase().includes(term)
      );
    };
  ...
  return (
    <div className="result-display">
      <div className="result-header">
        ...
        <h2>분석 결과</h2>
          {analysisTime && <p className="analysis-time">분석 소요 시간: {analysisTime}</p>}
        </div>
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    {(() => {
                      ...
                      filteredData.customer_based_orders.slice(0, 1000).forEach(order => {
                        const customer = order?.customer || '미지정';
                      ...
                      // 그룹화된 데이터로 테이블 렌더링
                      let rows = [];
                      Object.keys(customerGroups).forEach((customer, groupIndex) => {
                        const orders = customerGroups[customer];
                        const groupClass = groupIndex % 2 === 0 ? 'even-group' : 'odd-group';
                        
                        orders.forEach((order, orderIndex) => {
                          rows.push(
                            <tr key={`customer-${groupIndex}-${orderIndex}`} className={groupClass}>
                              {orderIndex === 0 && (
                                <td rowSpan={orders.length} className="customer-cell">
                                  {customer}
                                </td>
                              )}
      ...
      <div className="back-to-main-container">
        <button 
          ...
          className="btn btn-primary back-to-main-btn"
        >
          메인으로 돌아가기
        </button>
      </div>
