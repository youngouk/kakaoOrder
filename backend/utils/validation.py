...
def is_valid_order_format(order: Dict[str, Any]) -> bool:
    ...
    # 품목명이나 주문 노트에 '취소'가 포함된 경우 취소 주문으로 간주
    if item and "취소" in item:
        is_cancellation = True
    if note and "취소" in note:
        is_cancellation = True
    
    # 취소 주문의 경우 품목명에서 '취소' 키워드가 포함되어도 유효함
    if is_cancellation:
        # 취소 주문은 customer와 item만 필수
        if not order.get("customer"):
            print(f"취소 주문 검증 실패: 고객명 누락")
            return False
        return True
...
def filter_invalid_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        # 취소 관련 항목은 보존 (나중에 처리하기 위해)
        note = item.get("note", "").lower() if item.get("note") else ""
        item_name = item.get("item", "")
        is_cancellation = (item_name and "취소" in item_name) or (note and "취소" in note)
