...
CLAUDE_MODEL = "claude-sonnet-4-20250514"  # 개선된 모델 사용
...
"""
            ...
            response = client.messages.create(
                ...
                tools=tools,
                temperature=0.3,
                max_tokens=4000  # 타임아웃 방지를 위해 줄임
            )
            ...
            if "products" in result and isinstance(result["products"], list):
                ...
                logger.info(f"LLM에서 {len(products)}개 상품 추출 성공: {[product['name'] for product in products]}")
                return products
            else:
